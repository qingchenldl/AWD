...
public function createNoncestr( $length = 32 ) 
	...
	public function getSign($Obj)
	{
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".$content['wxapikey'];
	...
	function arrayToXml($arr)
    {
        ...
        $xml.="</xml>";
		//$xml = mb_convert_encoding($xml,'ISO8859-1');
        return $xml; 
    }
...
{	
	...
	{	
		global $content;
		$this->appid=$content['wxappid'];
		$this->secret=$content['AppSecret'];
		$this->mchid=$content['wxmchid'];
		//设置接口链接
...
class NativeLink_pub  extends Common_util_pub
{
	...
	function createLink()
	{
		...
		{		
			...
			//$this->parameters["product_id"] = urlencode($this->parameters["product_id"]);
		   	$this->parameters["appid"] = WxPayConf_pub::APPID;//公众账号ID
		   	$this->parameters["mch_id"] = WxPayConf_pub::MCHID;//商户号
		   	$time_stamp = time();
	...
	{		
		...
		return iconv("ISO8859-1", "UTF-8", $this->url);
